...
export class HttpService {
  ...
  convertUrl(url) {
    return url.startsWith('http')?url:(this._url+url);
  }

  get(url: string, param?: any): Observable<any> {
    let options = this.setRequestOptions(param);
    console.log(this.convertUrl(url), options)
    return this.http.get(this.convertUrl(url), options)
  ...
  post(url: string, body: any, param?: any): Observable<any> {
    ...
    let options = this.setRequestOptions(param);
    return this.http.post(this.convertUrl(url), bodyString, options)
      .map((res:Response) => res.json())
      .catch((error: any) => this.handleError(error))
  }
  put(url: string, body: any): Observable<any> {
    ...
    return this.http.put(this.convertUrl(url), bodyString, options)
      .map((res:Response) => res.json())
      .catch((error: any) => this.handleError(error))
  }
  remove(url: string): Observable<any> {
    return this.http.delete(this.convertUrl(url))
      .map((res:Response) => res.json())
      .catch((error: any) => this.handleError(error))
  }
  patch(url: string, body: any): Observable<any> {
    let bodyString = JSON.stringify(body);
    let options = this.setRequestOptions();
    return this.http.patch(this.convertUrl(url), bodyString, options)
      .map((res:Response) => res.json())
      .catch((error: any) => this.handleError(error))
  }
